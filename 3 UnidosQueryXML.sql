...
Inner Join ITG_ENC2 G
on C.NUMSERIE = G.NUMSERIE_ENC AND C.NUMALBARAN = G.NUMALBARAN_ENC
where C.NUMSERIE='AAAY' AND C.NUMFAC='1342'; 
--POSICIONES
DECLARE @CANJEO INT 
SET @CANJEO = (SELECT VALOR   FROM [01-SVRWSPRUEBA\MNG].[ICGTIERRAGRO2017].dbo.[PARAMETROS]   WHERE CLAVE = 'FIDEL' AND SUBCLAVE = 'EDTOS')
...
CAST(B.UNIDADESTOTAL AS INT) as Cantidad, 
...
CAST(B.precio * B.UNIDADESTOTAL AS INT) AS PrecioBaseTotal,
'Z024' AS DsctoAuto,
'0' AS Dscto,
'0' AS ValorDscto,
...
where A.NUMSERIE='AAAY' AND A.NUMFAC='1343' SELECT * FROM ZZPRUEBA_ENC, ZZPRUEBADET, ZZRECAUDOSXML FOR XML RAW('Integracion'), ELEMENTS	
--RECAUDOS
--Recaudos CABECERA
...
where A.NUMSERIE='AAAY' AND A.NUMFAC='1343';
--RecaudosMedioDePago
Insert into ZZRECAUDOSXML
Select top 1 
'P' as Campo03,
...
inner Join ALBVENTALIN H
on A.NUMSERIE = H.NUMSERIE AND A.NUMALBARAN = H.NUMALBARAN
where A.NUMSERIE='AAAY' AND A.NUMFAC='1343' GROUP BY A.NUMSERIEFAC,E.CODTIPOPAGO,D.CTA_EFECTIVO,F.CUENTACOBRO,E.IMPORTE,G.RAIZCOBROS,D.SAP_OFICINAS_VENTAS,G.RAIZPAGOS,D.SAP_GRUPO_VENDEDOR;
...
inner Join ALBVENTALIN H
on A.NUMSERIE = H.NUMSERIE AND A.NUMALBARAN = H.NUMALBARAN
where A.NUMSERIE='AAAY' AND A.NUMFAC='1343' GROUP BY A.NUMSERIEFAC, C.CODCLIENTESAP, E.IMPORTE, G.RAIZCOBROS, D.SAP_OFICINAS_VENTAS, G.RAIZPAGOS, D.SAP_GRUPO_VENDEDOR;